...
class Webservices
{
    ...
    public function horario_ciclo_actual_alumno()
    {
        ...
        for ($i = 0; $i < $tamano; $i++) {
            ...
            for ($b = 0; $b < $tamano_int; $b++) {
                ...
                if (intval($strDate) >= intval($class_date) && intval($strDate) < intval($class_end_date) && strlen($enable_survey) !== 0) 
                {
    ...
    public function encuestas_del_dia()
    {
        ...
        $tamano = count($json['HorarioDia']);
        $i = 0;
        $date = new DateTime(date("now"), new DateTimeZone('America/Lima'));
        $today = $date->format('N');
        for ($j = 0; $j <= $tamano; $j++) {
...
//                }
    ...
    public function reserva_recursos()
    {
        ...
        if ($codrecurso == "{post:CodRecurso}" || $nomrecurso == "{post:NomRecurso}") {
            ...
            if ($json['CodError'] === "00000") {
                ...
                if ($error === "00002") {
                    $error_mensaje = "Lo sentimos, el recurso que intentas reservar ya está ocupado, realiza una nueva búsqueda.";
                }
                $result .= '<span class="helvetica-16 red">' . $error_mensaje . '</span>';
                $result .= '</div>';
                $result .= "</div>";
    ...
    public function activar_recurso_computadora()
    {
        $code_reserva = ee()->TMPL->fetch_param('cod_reserva');
...
//                    }
    ...
    // FULL CODIGO ALUMNO
    public function full_user_code()
    {
        return $this->upc_user_data->get_full_user_code();
    }
...
//        $this->services->upc_log("WFSENTHORARIO;" . $quiz_result . ";" . date('ddmmyyyy - H:i:s') . "\n", "logs/WSENTHORARIO.log");
    ...
    public function get_apoderado_user_data_update(){
        $tipo = ee()->TMPL->fetch_param('tipo');
    
        $regsrv = $this->upc_services->get_data_update_parentesco($tipo);
        $array = array();
        if($regsrv != false && $regsrv->DTOHeader->CodigoRetorno != NULL)
        {
            $ficha = $regsrv->ListaDTOParentesco[0];

            $array = array('NombreApoderado' => $ficha->DTOParentescoDetalle[0]->NombreApoderado,
                    'ApellidoPaterApoderado'=> $ficha->DTOParentescoDetalle[0]->ApellidoPaterApoderado,
                    'ApellidoMaterApoderado'=> $ficha->DTOParentescoDetalle[0]->ApellidoMaterApoderado);
